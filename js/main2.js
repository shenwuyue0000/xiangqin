...
function handlePhoneLogin() {
    ...
    if (registered) {
        showToast(`欢迎回来！手机号 ${phone.slice(0,3)}****${phone.slice(-4)} 登录成功`, false);
    } else {
        addRegisteredPhone(phone);
        showToast(`注册成功！手机号 ${phone.slice(0,3)}****${phone.slice(-4)} 已加入良缘荟`, false);
    }

    // 设置当前用户（模拟手机登录用户）
    currentUser = {
        id: phone,
        name: `手机用户 ${phone.slice(0,3)}****${phone.slice(-4)}`,
        email: `${phone}@phone.local`,
        provider: 'phone',
        providerIcon: 'fas fa-mobile-alt'
    };
    // 存储到 localStorage（模拟持久化，便于刷新后仍然保持手机登录）
    localStorage.setItem('phone_user', JSON.stringify(currentUser));
    updateNavBar();
    closePhoneModal();
}
